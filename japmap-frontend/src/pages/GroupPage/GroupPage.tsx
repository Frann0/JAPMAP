...
const GroupIDPage = observer(() => {
  ...
  const { mapStore, authStore, groupStore } = useStore();
  useEffect(() => {
  ...
  const addProject = async () => {
    ...
    await groupStore.addProjectToGroup(Number.parseInt(params.id), authStore.user!.uid, input,).then(() => {
      setInput("");
      setShowModal(false);
      setLoading(false);
    });
    ...
    {groupStore.selectedGroup &&
      <>
        ...
        <div className="Project">
          <Title title={`Gruppeprojekter - ${groupStore.selectedGroup.name}`} />
          ...
          <div className="Project_Content">
            {groupStore.selectedGroupProjects.map((project) => (
              <MapComponent key={project.id} map={project} toggle={() => groupStore.toggleProject(project.project.id)} />
            ))}
          </div>
        </div>
      </>
